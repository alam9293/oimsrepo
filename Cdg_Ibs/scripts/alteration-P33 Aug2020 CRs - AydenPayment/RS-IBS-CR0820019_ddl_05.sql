...
select  'RS-IBS-CR0820019_ddl_05_' || sys_context('userenv','instance_name') || '_' || to_char(sysdate,'YYYYMMDD_HH24MISS') dt,'.txt' ext from dual; 
...
DROP SEQUENCE SQ_TMTB_NON_BILLABLE_CRCA_REQ;
...
comment on column TMTB_NON_BILLABLE_TXN_CRCA_REQ.STATUS is 'The status of the process which state if the process is running or have an error';
