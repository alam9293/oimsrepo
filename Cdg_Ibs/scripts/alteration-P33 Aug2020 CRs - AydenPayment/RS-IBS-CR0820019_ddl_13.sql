...
select  'RS-IBS-CR0820019_ddl_13_' || sys_context('userenv','instance_name') || '_' || to_char(sysdate,'YYYYMMDD_HH24MISS') dt,'.txt' ext from dual;
...
ADD FILE_NAME VARCHAR2(50);

ALTER TABLE TMTB_NON_BILLABLE_TXN_CRCA
ADD UPLOAD_DATE DATE;

comment on column TMTB_NON_BILLABLE_TXN_CRCA.FILE_NAME is 'The file that is used to matched the content of the TMTB_NON_BILLABLE_TXN. The content is uploaded by user in the adyen portal';
comment on column TMTB_NON_BILLABLE_TXN_CRCA.UPLOAD_DATE is 'The date where the file is uploaded';
