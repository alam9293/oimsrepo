...
public class AddDeptWindow extends CommonInnerAcctWindow {
	private static final long serialVersionUID = 1L;
	private static final Logger logger = Logger.getLogger(AddDeptWindow.class);
	...
	public AddDeptWindow() throws InterruptedException{
		logger.info("AddDeptWindow()");
		//customer number
	...
	public void init() throws InterruptedException{
		...
		if(contacts.isEmpty()){
			Messagebox.show("No contact person! Please create contact person first.", "Create Department", Messagebox.OK, Messagebox.ERROR);
			this.back();
			return;
		}
		...
		// now getting the divisions
		Map<Integer, Map<String, String>> divisions = this.businessHelper.getAccountBusiness().searchAccounts(custNo, null, null, null, null, NonConfigurableConstants.DIVISION_LEVEL, null, null);
		if(!divisions.isEmpty()){
			Listbox divisionBox = (Listbox)this.getFellow("divisionList");
			for(Integer acctNo : divisions.keySet()){
				String display = divisions.get(acctNo).get("acctName") + "(" + divisions.get(acctNo).get("acctCode") + ")";
				divisionBox.appendChild(new Listitem(display, divisions.get(acctNo).get("acctCode")));
			}
			divisionBox.setSelectedIndex(0);
			updateNameOnCard(divisionBox.getSelectedItem());
			updateProductSubscriptions(divisionBox.getSelectedItem());
		}else{
			Messagebox.show("No division found. Please create division first.", "Create Department", Messagebox.OK, Messagebox.EXCLAMATION);
			this.back();
		}
	}
	public void save() throws InterruptedException, CniiInterfaceException{
		logger.info("save()");
		if(Messagebox.show("Create Department?", "Create Department", Messagebox.OK | Messagebox.CANCEL, Messagebox.QUESTION)==Messagebox.OK){
			try
			{
			...
			if(prodSubs.isEmpty()){
				Messagebox.show("No product subscriptions!", "Create Department", Messagebox.OK, Messagebox.EXCLAMATION);
				return;
			}
			accountDetails.put("prodSubscriptions", prodSubs);
			// checking credit limit
			String divisionCode = (String)((Listbox)this.getFellow("divisionList")).getSelectedItem().getValue();
			BigDecimal deptCreditLimit = ((Decimalbox)this.getFellow("creditLimit")).getValue();
			accountDetails.put("creditLimit", deptCreditLimit);
			if(this.businessHelper.getAccountBusiness().checkParentCreditLimit(custNo, divisionCode, deptCreditLimit)){
				Messagebox.show("Credit Limit is greater than Division!", "Create Department", Messagebox.OK, Messagebox.ERROR);
				return;
			}
			String deptCode = ((Textbox)this.getFellow("code")).getValue();
			accountDetails.put("acctCode", deptCode);
			// checking code
			if(this.businessHelper.getAccountBusiness().checkCode(custNo, NonConfigurableConstants.DEPARTMENT_LEVEL, divisionCode, deptCode)){
				Messagebox.show("Department code already in used! Please use another code", "Create Department", Messagebox.OK, Messagebox.ERROR);
				return;
			}
			// account
			accountDetails.put("parentCode", divisionCode);
			accountDetails.put("userId", getUserId());
			String deptName = ((Textbox)this.getFellow("acctName")).getValue();
			accountDetails.put("acctName", deptName);
			String deptNameOnCard = ((Textbox)this.getFellow("nameOnCard")).getValue();
			accountDetails.put("nameOnCard", deptNameOnCard);
...
//			}
			...
			{
				Messagebox.show("Government eInvoice & PUBBS should not double invoice", "Add Department", Messagebox.OK, Messagebox.ERROR);
				return;
			}
			...
			if(this.businessHelper.getAccountBusiness().createCorpSubAccount(accountDetails)){
				Messagebox.show("Department Created.", "Create Department", Messagebox.OK, Messagebox.INFORMATION);
				this.back();
			}else{
				Messagebox.show("Unable to create department! Please try again later", "Create Department", Messagebox.OK, Messagebox.ERROR);
			}
			
			}
			catch(IBSException e){
				Messagebox.show(e.getMessage(), "Create Department", Messagebox.OK , Messagebox.INFORMATION);
			}
			catch(Exception e){
				Messagebox.show("Unable to create department! Please try again later", "Create Department", Messagebox.OK, Messagebox.ERROR);
	...
	@SuppressWarnings("unchecked")
	public void updateProductSubscriptions(Listitem selectedItem){
		String divCode = (String)selectedItem.getValue();
		// initing the product subscriptions
		Rows rows = (Rows)this.getFellow("prodSubscriptions");
		Set<Map<String, String>> prodSubscriptions = this.businessHelper.getAccountBusiness().getProductSubscriptions(custNo, divCode);
	...
	public void updateNameOnCard(Listitem selectedDivision){
		String divisionCode = (String)selectedDivision.getValue();
		Map<String, Object> parentDetail = this.businessHelper.getAccountBusiness().getAccount(custNo, NonConfigurableConstants.DIVISION_LEVEL, null, divisionCode);
