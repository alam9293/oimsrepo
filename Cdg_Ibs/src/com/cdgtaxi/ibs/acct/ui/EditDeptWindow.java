...
public class EditDeptWindow extends AddDeptWindow {
	private static final long serialVersionUID = 1L;
	private static final Logger logger = Logger.getLogger(EditDeptWindow.class);
	private String custNo, acctStatus, parentCode, code;
	...
	public EditDeptWindow() throws InterruptedException {
		super();
		logger.info("EditDeptWindow()");
		//customer number
		Map<String, String> map = Executions.getCurrent().getArg();
		custNo = map.get("custNo");
		if(custNo==null || custNo.trim().length()==0){
			Messagebox.show("No account number found!", "Edit Department", Messagebox.OK, Messagebox.ERROR);
		}
		// account status
		acctStatus = map.get("acctStatus");
		if(acctStatus==null || acctStatus.trim().length()==0){
			Messagebox.show("No account status found!", "Edit Department", Messagebox.OK, Messagebox.ERROR);
		}
		// account code
		code = map.get("code");
		if(code==null || code.trim().length()==0){
			Messagebox.show("No account code found!", "Edit Department", Messagebox.OK, Messagebox.ERROR);
		}
		// parent code
		parentCode = map.get("parentCode");
		if(parentCode==null || parentCode.trim().length()==0){
			Messagebox.show("No parent code found!", "Edit Department", Messagebox.OK, Messagebox.ERROR);
		}
	...
	public void init() throws InterruptedException{
		...
		if(acctStatus.equals(NonConfigurableConstants.ACCOUNT_STATUS.get(NonConfigurableConstants.ACCOUNT_STATUS_PENDING_ACTIVATION))){
			...
			((Listbox)this.getFellow("divisionList")).setDisabled(false);
		}else{
			((Listbox)this.getFellow("divisionList")).setDisabled(true);
			((Textbox)this.getFellow("code")).setDisabled(true);
			((Decimalbox)this.getFellow("creditLimit")).setDisabled(true);
		}
		Map<String, Object> acctDetails = this.businessHelper.getAccountBusiness().getAccount(custNo, 2, parentCode, code);
		this.acctNo = (Integer)acctDetails.get("acctNo");
		Listbox divisionList = (Listbox)this.getFellow("divisionList");
		for(Object division : divisionList.getItems()){
			if(((Listitem)division).getValue().equals(parentCode)){
				((Listitem)division).setSelected(true);
				this.updateProductSubscriptions((Listitem)division);
			}
		}
	...
	public void save() throws InterruptedException, CniiInterfaceException{
		...
		try
		{
		if(Messagebox.show("Save Department?", "Edit Department", Messagebox.OK | Messagebox.CANCEL, Messagebox.QUESTION)==Messagebox.OK){
			...
			if(prodSubs.isEmpty()){
				Messagebox.show("No product subscriptions!", "Edit Department", Messagebox.OK, Messagebox.EXCLAMATION);
				return;
			}
...
//			accountDetails.put("acctNo", acctNo);
			...
			{
				Messagebox.show("Government eInvoice & PUBBS should not double invoice", "Edit Department", Messagebox.OK, Messagebox.ERROR);
				return;
			}
			...
			if (null != result.get("result") && result.get("result").toString().trim().equals("true")){
				Messagebox.show("Department Saved.", "Edit Department", Messagebox.OK, Messagebox.INFORMATION);
				this.back();
			}else{
				Messagebox.show("Unable to save department! Please try again later", "Edit Department", Messagebox.OK, Messagebox.ERROR);
			}
		}
		}
		catch(CniiInterfaceException e) {
			Messagebox.show(e.getMessage(), "Edit Department", Messagebox.OK , Messagebox.INFORMATION);
		}
	...
	public void delete() throws InterruptedException{
		if(Messagebox.show("Confirm Delete?", "Edit Department", Messagebox.OK | Messagebox.CANCEL, Messagebox.QUESTION) == org.zkoss.zhtml.Messagebox.OK){
			this.businessHelper.getAccountBusiness().deleteSubAccount(acctNo, getUserLoginIdAndDomain());
			Messagebox.show("Department deleted", "Edit Department", Messagebox.OK , Messagebox.INFORMATION);
			this.back();
		}
	}
