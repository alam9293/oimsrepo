...
public class ViewDeptWindow extends CommonInnerAcctWindow {
	private static final long serialVersionUID = 1L;
	private static final Logger logger = Logger.getLogger(ViewDeptWindow.class);
	private String custNo, acctStatus, parentCode, code;
	@SuppressWarnings("unchecked")
	public ViewDeptWindow() throws InterruptedException{
		logger.info("ViewDeptWindow()");
		//customer number
		Map<String, String> map = Executions.getCurrent().getArg();
		custNo = map.get("custNo");
		if(custNo==null || custNo.trim().length()==0){
			Messagebox.show("No account number found!", "View Department", Messagebox.OK, Messagebox.ERROR);
		}
		// account status
		acctStatus = map.get("acctStatus");
		if(acctStatus==null || acctStatus.trim().length()==0){
			Messagebox.show("No account status found!", "View Department", Messagebox.OK, Messagebox.ERROR);
		}
		// account code
		code = map.get("code");
		if(code==null || code.trim().length()==0){
			Messagebox.show("No account code found!", "View Department", Messagebox.OK, Messagebox.ERROR);
		}
		// parent code
		parentCode = map.get("parentCode");
		if(parentCode==null || parentCode.trim().length()==0){
			Messagebox.show("No parent code found!", "View Department", Messagebox.OK, Messagebox.ERROR);
		}
	}
	@SuppressWarnings("unchecked")
	public void init() throws InterruptedException{
		...
		if(acctDetails.get("acctNo")==null){
			this.back();
			return;
		}
		Label division = (Label)this.getFellow("division");
		division.setValue((String)acctDetails.get("parentName"));
		...
		Label deptStatus = (Label)this.getFellow("deptStatus");
		deptStatus.setValue(NonConfigurableConstants.ACCOUNT_STATUS.get(acctDetails.get("childStatus")));
		enableAllBtns();
		...
		Set<Map<String, String>> prodSubscriptions = this.businessHelper.getAccountBusiness().getProductSubscriptions(custNo, parentCode);
		...
		if(!this.checkUriAccess(Uri.EDIT_DEPT))
			((Button)this.getFellow("editBtn")).setDisabled(true);
		if(!this.checkUriAccess(Uri.CREDIT_REVIEW_DEPT))
			((Button)this.getFellow("creditReviewBtn")).setDisabled(true);
		if(!this.checkUriAccess(Uri.SUSPEND_DEPT))
			((Button)this.getFellow("suspendBtn")).setDisabled(true);
		if(!this.checkUriAccess(Uri.REACTIVATE_DEPT))
			((Button)this.getFellow("reactivateBtn")).setDisabled(true);
		if(!this.checkUriAccess(Uri.TERMINATE_DEPT))
			((Button)this.getFellow("terminateBtn")).setDisabled(true);
...
//			((Label)this.getFellow("fiFlag")).setValue(NonConfigurableConstants.BOOLEAN_YN.get(acctDetails.get("fiFlag")));
	...
	public void viewStatusHistory() throws SuspendNotAllowedException, InterruptedException{
		super.viewStatusHistory(custNo, null, parentCode, code);
	}
