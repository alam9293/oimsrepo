...
public class ManageIssuanceFeePlanWindow extends CommonWindow {
	private static final Logger logger = Logger.getLogger(ManageIssuanceFeePlanWindow.class);
	...
	public void onCreate(CreateEvent ce) throws Exception {
		...
		itemTemplate.detach();
		refresh();
		if(!this.checkUriAccess(Uri.CREATE_ISSUANCE_FEE_PLAN)){
	...
	public void editPlan() throws InterruptedException {
		try {
			...
			MstbIssuanceFeeMaster plan = (MstbIssuanceFeeMaster)selectedItem.getValue();

			HashMap<String, Object> map = new HashMap<String, Object>();
			map.put("planNo", plan.getIssuanceFeeNo());
			
			if(this.checkUriAccess(Uri.EDIT_ISSUANCE_FEE_PLAN))
				forward(Uri.EDIT_ISSUANCE_FEE_PLAN, map);
			else if(this.checkUriAccess(Uri.VIEW_ISSUANCE_FEE_PLAN))
				forward(Uri.VIEW_ISSUANCE_FEE_PLAN, map);
			else{
	...
	public void createPlan() throws InterruptedException {
		try {
			forward(Uri.CREATE_ISSUANCE_FEE_PLAN, null);
		} 
		catch(Exception e){
			Messagebox.show(com.cdgtaxi.ibs.common.NonConfigurableConstants.COMMON_ERROR_MESSAGE,
					"Error", Messagebox.OK, Messagebox.ERROR);
	...
	public void deletePlan(Listitem item) throws InterruptedException {
		if(!this.checkUriAccess(Uri.DELETE_ISSUANCE_FEE_PLAN)){
			Messagebox.show(com.cdgtaxi.ibs.common.NonConfigurableConstants.ACCESS_DENIED,
					"Access Denied", Messagebox.OK, Messagebox.EXCLAMATION);
			return;
		}
		...
		try {
			businessHelper.getAdminBusiness().deleteIssuanceFeePlan((MstbIssuanceFeeMaster) item.getValue());
			MasterSetup.getIssuanceManager().refresh();
			item.detach();
			Messagebox.show("Plan has been successfully deleted", "Delete Issuance Fee Plan",
	...
	public void refresh() throws InterruptedException {
		List<MstbIssuanceFeeMaster> plans = businessHelper.getAdminBusiness().getIssuanceFeePlans();
		...
		if (plans.size() == 0) {
			...
			for (MstbIssuanceFeeMaster plan : plans) {
				Listitem item = (Listitem) itemTemplate.clone();
				item.setValue(plan);
				List<Listcell> cells = item.getChildren();
				cells.get(0).setLabel(plan.getIssuanceFeeName());
				if(plan.getMstbIssuanceFeeDetails().size()<=0){
					Image deleteImage = new Image("/images/delete.png");
					deleteImage.setStyle("cursor:pointer");
					ZScript showInfo = ZScript.parseContent("manageIssuanceFeePlanWindow.deletePlan(self.getParent().getParent())");
