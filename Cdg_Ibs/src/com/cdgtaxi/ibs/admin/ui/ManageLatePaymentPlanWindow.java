...
public class ManageLatePaymentPlanWindow extends CommonWindow {
	private static final Logger logger = Logger.getLogger(ManageLatePaymentPlanWindow.class);
	...
	public void onCreate(CreateEvent ce) throws Exception {
		...
		itemTemplate.detach();
		refresh();
		if(!this.checkUriAccess(Uri.CREATE_LATE_PAYMENT_PLAN)){
	...
	public void editPlan() throws InterruptedException {
		try {
			...
			MstbLatePaymentMaster plan = (MstbLatePaymentMaster)selectedItem.getValue();

			HashMap<String, Object> map = new HashMap<String, Object>();
			map.put("planNo", plan.getLatePaymentPlanNo());
			
			if(this.checkUriAccess(Uri.EDIT_LATE_PAYMENT_PLAN))
				forward(Uri.EDIT_LATE_PAYMENT_PLAN, map);
			else if(this.checkUriAccess(Uri.VIEW_LATE_PAYMENT_PLAN))
				forward(Uri.VIEW_LATE_PAYMENT_PLAN, map);
			else{
	...
	public void createPlan() throws InterruptedException {
		try {
			forward(Uri.CREATE_LATE_PAYMENT_PLAN, null);
		}
		catch(Exception e){
			Messagebox.show(com.cdgtaxi.ibs.common.NonConfigurableConstants.COMMON_ERROR_MESSAGE,
					"Error", Messagebox.OK, Messagebox.ERROR);
	...
	public void deletePlan(Listitem item) throws InterruptedException {
		if(!this.checkUriAccess(Uri.DELETE_LATE_PAYMENT_PLAN)){
			Messagebox.show(com.cdgtaxi.ibs.common.NonConfigurableConstants.ACCESS_DENIED,
					"Access Denied", Messagebox.OK, Messagebox.EXCLAMATION);
			return;
		}
		...
		try {
			businessHelper.getAdminBusiness().deleteLatePaymentPlan((MstbLatePaymentMaster) item.getValue());
			MasterSetup.getLatePaymentManager().refresh();
			item.detach();
			Messagebox.show("Plan has been successfully deleted", "Delete Late Payment Plan",
	...
	public void refresh() throws InterruptedException {
		List<MstbLatePaymentMaster> plans = businessHelper.getAdminBusiness().getLatePaymentPlans();
		...
		if (plans.size() == 0) {
			...
			for (MstbLatePaymentMaster plan : plans) {
				Listitem item = (Listitem) itemTemplate.clone();
				item.setValue(plan);
				List<Listcell> cells = item.getChildren();
				cells.get(0).setLabel(plan.getLatePaymentPlanName());
				if(plan.getMstbLatePaymentDetails().size()<=0){
					Image deleteImage = new Image("/images/delete.png");
					deleteImage.setStyle("cursor:pointer");
					ZScript showInfo = ZScript.parseContent("manageLatePaymentPlanWindow.deletePlan(self.getParent().getParent())");
