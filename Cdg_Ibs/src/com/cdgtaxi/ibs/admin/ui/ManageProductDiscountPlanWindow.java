...
public class ManageProductDiscountPlanWindow extends CommonWindow {
	private static final Logger logger = Logger.getLogger(ManageProductDiscountPlanWindow.class);
	...
	public void onCreate(CreateEvent ce) throws Exception {
		...
		itemTemplate.detach();
		refresh();
		if(!this.checkUriAccess(Uri.CREATE_PRODUCT_DISCOUNT_PLAN)){
	...
	public void editPlan() throws InterruptedException {
		try {
			...
			MstbProdDiscMaster plan = (MstbProdDiscMaster)selectedItem.getValue();

			HashMap<String, Object> map = new HashMap<String, Object>();
			map.put("planNo", plan.getProductDiscountPlanNo());
			
			if(this.checkUriAccess(Uri.EDIT_PRODUCT_DISCOUNT_PLAN))
				forward(Uri.EDIT_PRODUCT_DISCOUNT_PLAN, map);
			else if(this.checkUriAccess(Uri.VIEW_PRODUCT_DISCOUNT_PLAN))
				forward(Uri.VIEW_PRODUCT_DISCOUNT_PLAN, map);
			else{
	...
	public void createPlan() throws InterruptedException {
		try {
			forward(Uri.CREATE_PRODUCT_DISCOUNT_PLAN, null);
		} 
		catch(Exception e){
			Messagebox.show(com.cdgtaxi.ibs.common.NonConfigurableConstants.COMMON_ERROR_MESSAGE,
					"Error", Messagebox.OK, Messagebox.ERROR);
	...
	public void deletePlan(Listitem item) throws InterruptedException {
		if(!this.checkUriAccess(Uri.DELETE_PRODUCT_DISCOUNT_PLAN)){
			Messagebox.show(com.cdgtaxi.ibs.common.NonConfigurableConstants.ACCESS_DENIED,
					"Access Denied", Messagebox.OK, Messagebox.EXCLAMATION);
			return;
		}
		...
		try {
			businessHelper.getAdminBusiness().deleteProductDiscountPlan((MstbProdDiscMaster) item.getValue());
			MasterSetup.getProductDiscountManager().refresh();
			item.detach();
			Messagebox.show("Plan has been successfully deleted", "Delete Product Discount Plan",
	...
	public void refresh() throws InterruptedException {
		List<MstbProdDiscMaster> plans = businessHelper.getAdminBusiness().getProductDiscountPlans();
		...
		if (plans.size() == 0) {
			...
			for (MstbProdDiscMaster plan : plans) {
				Listitem item = (Listitem) itemTemplate.clone();
				item.setValue(plan);
				List<Listcell> cells = item.getChildren();
				cells.get(0).setLabel(plan.getProductDiscountPlanName());
				
				if(plan.getMstbProdDiscDetails().size()<=0){
					Image deleteImage = new Image("/images/delete.png");
					deleteImage.setStyle("cursor:pointer");
					ZScript showInfo = ZScript.parseContent("manageProductDiscountPlanWindow.deletePlan(self.getParent().getParent())");
