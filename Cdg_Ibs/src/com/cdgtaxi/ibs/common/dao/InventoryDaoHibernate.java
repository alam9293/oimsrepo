...
public class InventoryDaoHibernate extends GenericDaoHibernate implements InventoryDao {
	...
	public ImtbItemType getItemType(Integer itemTypeNo) {
		...
		//DetachedCriteria issueReqCriteria = itemTypeCriteria.createCriteria("imtbIssueReqs", DetachedCriteria.LEFT_JOIN);
	...
	public List<ImtbStock> getStocksByItemType(Integer itemTypeNo){
		...
		DetachedCriteria itemTypeCrit = stockCrit.createCriteria("imtbItemType", DetachedCriteria.LEFT_JOIN);
	...
	private Query getItemTypeDtoQuery(Integer itemTypeNo) {
		Query query = getSession().getNamedQuery("getInventoryItemTypeDto");
	...
	public ImtbIssueReq getIssuanceRequest(Integer requestId) {
		...
		DetachedCriteria issueCriteria =
			requestCriteria.createCriteria("imtbIssue", DetachedCriteria.LEFT_JOIN);
	...
	public boolean serialNumberAvailable(Integer itemTypeNo,
		...
		Query query = getSession().getNamedQuery("serialNoAvailable");
		query.setString("stockInCode", NonConfigurableConstants.STOCK_TXN_TYPE_IN);
		query.setString("issuedCode", NonConfigurableConstants.STOCK_TXN_TYPE_ISSUED);
		query.setInteger("itemTypeNo", itemTypeNo);
	...
	public ImtbIssue getIssuanceByStockNo(Integer stockNo) {
		...
		DetachedCriteria stockCriteria =
			issueCriteria.createCriteria("imtbStock", DetachedCriteria.LEFT_JOIN);
		stockCriteria.add(Restrictions.idEq(stockNo));

		// Fetch request
		DetachedCriteria requestCriteria =
			issueCriteria.createCriteria("imtbIssueReq", DetachedCriteria.LEFT_JOIN);
		requestCriteria.setFetchMode("imtbItemType", FetchMode.JOIN);
		requestCriteria.setFetchMode("imtbIssueReqFlow", FetchMode.JOIN);
	...
	public List<Object[]> searchIssuanceRequest(SearchIssuanceRequestForm form) {
		...
		Query query = getSession().getNamedQuery("searchIssuanceRequest");
	...
	public List<ImtbItem> searchItem(SearchItemForm form){
		...
		DetachedCriteria itemTypeCrit = itemCrit.createCriteria("imtbItemType", DetachedCriteria.LEFT_JOIN);
		DetachedCriteria issueCrit = itemCrit.createCriteria("imtbIssue", DetachedCriteria.LEFT_JOIN);
		DetachedCriteria issueReqCrit = issueCrit.createCriteria("imtbIssueReq", DetachedCriteria.LEFT_JOIN);
		DetachedCriteria account3Crit = issueReqCrit.createCriteria("amtbAccount", DetachedCriteria.LEFT_JOIN);
		DetachedCriteria account2Crit = account3Crit.createCriteria("amtbAccount", DetachedCriteria.LEFT_JOIN);
		DetachedCriteria account1Crit = account2Crit.createCriteria("amtbAccount", DetachedCriteria.LEFT_JOIN);
	...
	public List<ImtbItemReq> searchItemRequest(SearchItemRequestForm form){
		...
		DetachedCriteria itemReqFlowCrit = itemReqCrit.createCriteria("imtbItemReqFlows", DetachedCriteria.LEFT_JOIN);
		DetachedCriteria itemCrit = itemReqCrit.createCriteria("imtbItem", DetachedCriteria.LEFT_JOIN);
		DetachedCriteria itemTypeCrit = itemCrit.createCriteria("imtbItemType", DetachedCriteria.LEFT_JOIN);
		DetachedCriteria issueCrit = itemCrit.createCriteria("imtbIssue", DetachedCriteria.LEFT_JOIN);
		DetachedCriteria issueReqCrit = issueCrit.createCriteria("imtbIssueReq", DetachedCriteria.LEFT_JOIN);
		DetachedCriteria account3Crit = issueReqCrit.createCriteria("amtbAccount", "acct3", DetachedCriteria.LEFT_JOIN);
		DetachedCriteria account2Crit = account3Crit.createCriteria("amtbAccount", "acct2", DetachedCriteria.LEFT_JOIN);
		DetachedCriteria account1Crit = account2Crit.createCriteria("amtbAccount", "acct1", DetachedCriteria.LEFT_JOIN);
		DetachedCriteria reasonCrit = itemReqCrit.createCriteria("mstbMasterTableByReason", DetachedCriteria.LEFT_JOIN);
