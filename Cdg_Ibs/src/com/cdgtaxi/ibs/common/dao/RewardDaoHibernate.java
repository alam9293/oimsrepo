...
public class RewardDaoHibernate extends GenericDaoHibernate implements RewardDao {
	...
	public List<LrtbRewardMaster> getAllRewardsPlan(){
		...
		DetachedCriteria detailCriteria = criteria.createCriteria("lrtbRewardDetails", DetachedCriteria.LEFT_JOIN);
		detailCriteria.createCriteria("lrtbRewardTiers", DetachedCriteria.LEFT_JOIN);
	...
	public LrtbRewardMaster getPlanWithDetails(Integer planNo) {
		...
		planCriteria.createCriteria("lrtbRewardDetails", DetachedCriteria.LEFT_JOIN);
		planCriteria.add(Restrictions.idEq(planNo));
	...
	public LrtbGiftCategory getCategoryWithItems(Integer giftCategoryNo) {
		...
		categoryCriteria.createCriteria("lrtbGiftItems", DetachedCriteria.LEFT_JOIN);
	...
	public AmtbAccount getAccountWithUsablePoints(Integer accountNo) {
		...
		accountCriteria.createAlias("amtbAcctStatuses", "sts", DetachedCriteria.LEFT_JOIN);
		accountCriteria.createCriteria("amtbContactPersons", DetachedCriteria.LEFT_JOIN);
		...
		DetachedCriteria issueCriteria = accountCriteria.createCriteria("lrtbRewardAccounts", DetachedCriteria.LEFT_JOIN);
		issueCriteria.createCriteria("lrtbRewardTxns", DetachedCriteria.LEFT_JOIN);
		issueCriteria.add(
	...
	public List<LrtbGiftStock> getRedemptionHistory(Integer accountNo) {
		DetachedCriteria stockCriteria = DetachedCriteria.forClass(LrtbGiftStock.class);
		DetachedCriteria txnCriteria = stockCriteria.createCriteria("lrtbRewardTxns", DetachedCriteria.LEFT_JOIN);
		...
		txnCriteria.createCriteria("amtbContactPerson", DetachedCriteria.LEFT_JOIN);
		accountCriteria.add(Restrictions.idEq(accountNo));
	...
	public Date getLatestToDate(Integer planNo) {
		DetachedCriteria planCriteria = DetachedCriteria.forClass(LrtbRewardMaster.class);
		planCriteria.createCriteria("lrtbRewardDetails", "d", DetachedCriteria.LEFT_JOIN);
		planCriteria.add(Restrictions.idEq(planNo));
	...
	public LrtbRewardTxn getInitialRewardPointsTxn(LrtbRewardAccount rewardAcct){
		...
		DetachedCriteria rewardAcctCriteria = criteria.createCriteria("lrtbRewardAccount", DetachedCriteria.LEFT_JOIN);
	...
	public List<LrtbRewardMaster> getOverlappingRewards(LrtbRewardDetail rewardDetail){
		...
		disjunction.add(Restrictions.sqlRestriction("? between effective_Dt_From and effective_Dt_To", rewardDetail.getEffectiveDtFrom(), Hibernate.TIMESTAMP));
		disjunction.add(Restrictions.sqlRestriction("? between effective_Dt_From and effective_Dt_To", rewardDetail.getEffectiveDtTo(), Hibernate.TIMESTAMP));
	...
	public BigDecimal getTotalRedeemedPoints(AmtbAccount account){
		...
		DetachedCriteria txn = criteria.createCriteria("lrtbRewardTxns", "txn", DetachedCriteria.LEFT_JOIN);
		DetachedCriteria stock = txn.createCriteria("lrtbGiftStock", DetachedCriteria.LEFT_JOIN);
	...
	public List<LrtbRewardAdjReq> getPendingAdjustmentRequests(){
		...
		requestCriteria.createCriteria("lrtbRewardAdjReqFlows", DetachedCriteria.LEFT_JOIN);
	...
	public List<LrtbRewardAdjReqFlow> searchAdjustmentRequest(SearchAdjustmentRequestForm form){
		...
		DetachedCriteria req = flow.createCriteria("lrtbRewardAdjReq", DetachedCriteria.LEFT_JOIN);
		DetachedCriteria childFlow = flow.createCriteria("lrtbRewardAdjReqFlows", "childFlow",DetachedCriteria.LEFT_JOIN);
		DetachedCriteria user = flow.createCriteria("satbUser", DetachedCriteria.LEFT_JOIN);
		DetachedCriteria acct = req.createCriteria("amtbAccount", DetachedCriteria.LEFT_JOIN);
		req.createCriteria("mstbMasterTable", DetachedCriteria.LEFT_JOIN);
