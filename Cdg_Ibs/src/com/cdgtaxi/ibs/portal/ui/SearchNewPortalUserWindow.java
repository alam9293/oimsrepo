...
public class SearchNewPortalUserWindow extends CommonWindow{

	private static final long serialVersionUID = 1L;
	private static final Logger logger = Logger.getLogger(SearchNewPortalUserWindow.class);
...
//		String userLoginId = ((CapsTextbox)this.getFellow("userLoginIdTextBox")).getValue();
		String userLoginId = "";
		String accessId = ((CapsTextbox)this.getFellow("accessIdTextBox")).getValue();
...
//		String masterLoginId = ((CapsTextbox)this.getFellow("masterLoginIdTextBox")).getValue();
		...
		try{
			...
			List<IttbCpLoginNew> logins = this.businessHelper.getPortalBusiness().searchNewPortalUser(
...
//			String noMaster = "";
			...
			if(logins.size()>0){
				...
				for(IttbCpLoginNew login : logins){
					...
					item.appendChild(newListcell(login.getAccessId()));
//					String stringLoginMethod = "-";
//					if(login.getLoginMethod() != null)
//					{
//						if(login.getLoginMethod().equalsIgnoreCase("C"))
//							stringLoginMethod = "Contact Person";
//						else if(login.getLoginMethod().equalsIgnoreCase("P"))
//							stringLoginMethod = "Product No";
//						else if(login.getLoginMethod().equalsIgnoreCase("M"))
//							stringLoginMethod = "Master Login";
//					}
//						
//					item.appendChild(newListcell(stringLoginMethod));
					
					
					AmtbAccount account;

					//skip M records
					if(login.getLoginMethod().equalsIgnoreCase("M"))
						continue;
					if(login.getAmtbContactPerson() == null  && login.getPmtbProduct() == null)
					{
						//skip 
						continue;
					}
...
//					item.appendChild(newListcell(login.getCreatedDt(), DateUtil.TRIPS_DATE_FORMAT));
	...
	public void reset(){
		logger.info("");
		((CapsTextbox)this.getFellow("accessIdTextBox")).setText("");
