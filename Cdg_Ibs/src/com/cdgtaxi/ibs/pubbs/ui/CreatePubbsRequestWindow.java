...
public class CreatePubbsRequestWindow extends CommonWindow implements AfterCompose {

	private static final long serialVersionUID = 1L;
	private static final Logger logger = Logger.getLogger(CreatePubbsRequestWindow.class);
	...
	public void createRequest() throws InterruptedException {
		...
		try {
			...
			IttbPubbsReq newRequest = new IttbPubbsReq();
			newRequest.setStatus(NonConfigurableConstants.PUBBS_REQUEST_STATUS_PENDING);
			form.status = NonConfigurableConstants.PUBBS_REQUEST_STATUS_PENDING;
			...
			if (this.businessHelper.getAdminBusiness().searchPubbsRequest(form).size() > 0) {
				throw new WrongValueException(
				...
				form.status = NonConfigurableConstants.PUBBS_REQUEST_STATUS_PENDING_PROGRESS;
				if (this.businessHelper.getAdminBusiness().searchPubbsRequest(form).size() > 0)
					throw new WrongValueException(
							"There is an existing pending/pending progress/in progress request with the same parameters. Creating the same request is not allowed until the previous request has been completed.");
				else {
					form.status = NonConfigurableConstants.PUBBS_REQUEST_STATUS_IN_PROGRESS;
					if (this.businessHelper.getAdminBusiness().searchPubbsRequest(form).size() > 0)
						throw new WrongValueException(
								"There is an existing pending/pending progress/in progress request with the same parameters. Creating the same request is not allowed until the previous request has been completed.");
				}
			...
			Messagebox.show("New request(" + requestNo + ") successfully created",
					"Create Pubbs Request", Messagebox.OK, Messagebox.INFORMATION);
