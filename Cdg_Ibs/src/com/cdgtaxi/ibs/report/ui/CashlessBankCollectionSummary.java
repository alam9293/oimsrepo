...
public class CashlessBankCollectionSummary extends ReportWindow implements CSVFormat {
	private static final long serialVersionUID = 1L;
	private static final Logger logger = Logger.getLogger(CashlessBankCollectionSummary.class);
	public CashlessBankCollectionSummary() throws IOException {
		super("Cashless Bank Collection Summary", "Non Billable");
	}
	@Override
	...
	public StringBuffer generateCSVData(Properties reportParamsProperties) throws FormatNotSupportedException {
		String batchNo = (String)reportParamsProperties.get("1. Batch No");
		String creditStart = (String)reportParamsProperties.get("2. Credit Start Date");
		String creditEnd = (String)reportParamsProperties.get("3. Credit End Date");
		String batchStart = (String)reportParamsProperties.get("4. Batch Start Date");
		String batchEnd = (String)reportParamsProperties.get("5. Batch End Date");
		String entityNo = (String)reportParamsProperties.get("6. Entity");
		String acquirerNo = (String)reportParamsProperties.get("7. Acquirer");
		String paymentTypeNo = (String)reportParamsProperties.get("8. Payment Type");
		String providerNo = (String)reportParamsProperties.get("9. Company Code");
		StringBuffer buffer = new StringBuffer();
		//Report Header
		buffer.append(Constants.TEXT_QUALIFIER+"Integrated Billing System"+Constants.TEXT_QUALIFIER+",");
		buffer.append("\n");
		buffer.append(Constants.TEXT_QUALIFIER+"Cashless Bank Collection Summary"+Constants.TEXT_QUALIFIER+",");
		buffer.append("\n");
		...
		if(paymentTypeNo!=null && paymentTypeNo.length()>0){
			String paymentTypeName = ((Listbox)this.getFellow("8. Payment Type")).getSelectedItem().getLabel();
			buffer.append(Constants.TEXT_QUALIFIER+"Payment Type: "+paymentTypeName+Constants.TEXT_QUALIFIER+",");
			buffer.append("\n");
		}
		if(providerNo!=null && providerNo.length()>0){
			String providerName = ((Listbox)this.getFellow("9. Company Code")).getSelectedItem().getLabel();
		...
		buffer.append(Constants.TEXT_QUALIFIER+"Total Records"+Constants.TEXT_QUALIFIER+",");
		...
		buffer.append(Constants.TEXT_QUALIFIER+"CR Records"+Constants.TEXT_QUALIFIER+",");
		...
		buffer.append(Constants.TEXT_QUALIFIER+"MDR (%)"+Constants.TEXT_QUALIFIER+",");
		buffer.append(Constants.TEXT_QUALIFIER+"Net Amount Received ($)"+Constants.TEXT_QUALIFIER+",");
		buffer.append(Constants.TEXT_QUALIFIER+"Rejected Records"+Constants.TEXT_QUALIFIER+",");
		buffer.append(Constants.TEXT_QUALIFIER+"Rejected Amount ($)"+Constants.TEXT_QUALIFIER+",");
		buffer.append(Constants.TEXT_QUALIFIER+"O/S from Bank ($)"+Constants.TEXT_QUALIFIER+",");
		buffer.append(Constants.TEXT_QUALIFIER+"Business Partner Commission ($)"+Constants.TEXT_QUALIFIER+",");
		buffer.append("\n");
		//Data
		List<Object[]> results = this.businessHelper.getReportBusiness().getCashlessBankCollectionSummary(
				batchNo,
				...
				entityNo, acquirerNo, paymentTypeNo, providerNo 
		);
