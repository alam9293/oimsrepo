...
public class CustomerAgingDetailWindow extends CommonWindow implements AfterCompose{
	
	private static final long serialVersionUID = 1L;
	private static final Logger logger = Logger.getLogger(CustomerAgingDetailWindow.class);
	
	private String report = "Customer Aging Detail";
	...
	public CustomerAgingDetailWindow() throws IOException{
	...
	public void afterCompose() {
		...
		Listbox salesPersonList = (Listbox)this.getFellow("salesPersonList");
		salesPersonList.appendChild(new Listitem("All", "All"));
		salesPersonList.setSelectedIndex(0);
		List<MstbSalesperson> salesPersons = this.businessHelper.getGenericBusiness().getAll(MstbSalesperson.class);
		for(MstbSalesperson salesPerson : salesPersons){
			salesPersonList.appendChild(new Listitem(salesPerson.getName(), salesPerson.getSalespersonNo()));
		}
		...
		typeList.appendChild(new Listitem("EXCESS", "R"));
		typeList.appendChild(new Listitem("INVOICE", "I"));
		typeList.setSelectedIndex(0);
		
		//populate typeList
		Listbox sortByList = (Listbox)this.getFellow("sortByList");
		sortByList.appendChild(new Listitem("ACCOUNT NAME / ID, TYPE, REF NO", "1"));
	...
	public void generate() throws HttpException, IOException, InterruptedException, NetException, WrongValueException {
		...
		Listbox salesPersonList = (Listbox)this.getFellow("salesPersonList");
		if(salesPersonList.getSelectedItem()!=null){
			if(!salesPersonList.getSelectedItem().getValue().toString().equals("All")){
				reportParamsProperties.put("5. Sales Person", salesPersonList.getSelectedItem().getValue().toString());
				salesPerson = salesPersonList.getSelectedItem().getValue().toString();
			}
		}
		...
		if(outstandingAmtDecimalBox.getValue()!=null){
			reportParamsProperties.put("6. Outstanding Amount", outstandingAmtDecimalBox.getValue().toString());
			outstandingAmount = outstandingAmtDecimalBox.getValue().toString();
		}
		
		Intbox noOfDaysLateIntBox = (Intbox)this.getFellow("noOfDaysLateIntBox");
		reportParamsProperties.put("7. No of Days Late", noOfDaysLateIntBox.getValue().toString());
		daysLate = noOfDaysLateIntBox.getValue().toString();
		
		Listbox typeList = (Listbox)this.getFellow("typeList");
		if(typeList.getSelectedItem()!=null){
			if(!typeList.getSelectedItem().getValue().toString().equals("All")){
				reportParamsProperties.put("8. Type", typeList.getSelectedItem().getValue().toString());
		...
		if(sortByList.getSelectedItem()!=null){
			reportParamsProperties.put("9. Sort By", sortByList.getSelectedItem().getValue().toString());
		...
		if(format.equals(Constants.FORMAT_CSV)){
			StringBuffer dataInCSV = this.generateCSVData(accountNo, accountName, entity, arControl, salesPerson, outstandingAmount, daysLate, type, sortBy, getUserLoginIdAndDomain());
	...
	private StringBuffer generateCSVData(String accountNo, String accountName, String entityNo,
		...
		sb.append("\n");
		sb.append(Constants.TEXT_QUALIFIER+"Customer Aging Report (Detail)"+Constants.TEXT_QUALIFIER+",");
		sb.append("\n");
		...
		if(salesPersonNo!=null && salesPersonNo.length()>0){
			MstbSalesperson salesPerson = (MstbSalesperson)MasterSetup.getSalespersonManager().getMaster(new Integer(salesPersonNo));
			sb.append(Constants.TEXT_QUALIFIER+"Sales Person: "+salesPerson.getName()+Constants.TEXT_QUALIFIER+",");
			sb.append("\n");
		}
		...
		if(daysLate!=null && daysLate.length()>0){
			sb.append(Constants.TEXT_QUALIFIER+"No. of Days Late >=: "+daysLate+Constants.TEXT_QUALIFIER+",");
			sb.append("\n");
		}
		if(type!=null && type.length()>0){
			...
			else if(type.equals("R")) value = "EXCESS";
		...
		sb.append(Constants.TEXT_QUALIFIER+"Invoice No / Receipt No / Note No"+Constants.TEXT_QUALIFIER+",");
		sb.append(Constants.TEXT_QUALIFIER+"Invoice / Receipt / Note Date"+Constants.TEXT_QUALIFIER+",");
		sb.append(Constants.TEXT_QUALIFIER+"Type"+Constants.TEXT_QUALIFIER+",");
		sb.append(Constants.TEXT_QUALIFIER+"No. of Days Late"+Constants.TEXT_QUALIFIER+",");
		...
		sb.append(Constants.TEXT_QUALIFIER+"Charge To"+Constants.TEXT_QUALIFIER+",");
		sb.append(Constants.TEXT_QUALIFIER+"Billing Contact Person"+Constants.TEXT_QUALIFIER+",");
		sb.append(Constants.TEXT_QUALIFIER+"Telephone"+Constants.TEXT_QUALIFIER+",");
		sb.append(Constants.TEXT_QUALIFIER+"Email"+Constants.TEXT_QUALIFIER+",");
		sb.append(Constants.TEXT_QUALIFIER+"Address"+Constants.TEXT_QUALIFIER+",");
		...
		//Data
		List<Object[]> rowsOfData = this.businessHelper.getReportBusiness().getCustomerAgingDetail(accountNo, accountName, entityNo, arControlCodeNo, salesPersonNo, outstandingAmount, daysLate, type, sortBy);
	...
	public void reset(){
		...
		((Listbox)this.getFellow("salesPersonList")).setSelectedIndex(0);
		((Decimalbox)this.getFellow("outstandingAmtDecimalBox")).setText("");
		((Intbox)this.getFellow("noOfDaysLateIntBox")).setText("0");
