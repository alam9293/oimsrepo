...
select  sys_context('userenv','instance_name') || '_septIssueLogs_ddl_03_' || to_char(sysdate,'YYYYMMDDHH24MISS') dt,'.txt' ext from dual;
...
CREATE INDEX PMIX_VIRT_EMAIL__ACCT_NO ON PMTB_VIRTUAL_EMAIL (ACCOUNT_NO);
...
COMMENT ON COLUMN PMTB_VIRTUAL_EMAIL.ACCOUNT_NO IS 'The Account No the email is tag to';
