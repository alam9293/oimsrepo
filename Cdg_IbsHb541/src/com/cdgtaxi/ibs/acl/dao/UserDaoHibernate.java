...
public class UserDaoHibernate extends GenericDaoHibernate implements UserDao, UserDetailsService{
	...
	public UserDetails loadUserByUsernameAndDomain(String loginId, String domain)
		...
		userCriteria.createCriteria("satbRoles", org.hibernate.sql.JoinType.LEFT_OUTER_JOIN);
		userCriteria.add(Restrictions.sqlRestriction("lower(login_Id) = lower(?)", loginId, StandardBasicTypes.STRING));
		userCriteria.add(Restrictions.sqlRestriction("lower(domain) = lower(?)", domain, StandardBasicTypes.STRING));
	...
	public List<SatbUser> searchUser(String uri){
		DetachedCriteria userCriteria = DetachedCriteria.forClass(SatbUser.class);
		userCriteria = userCriteria.createCriteria("satbRoles", org.hibernate.sql.JoinType.LEFT_OUTER_JOIN);
		userCriteria = userCriteria.createCriteria("satbResources", org.hibernate.sql.JoinType.LEFT_OUTER_JOIN);
