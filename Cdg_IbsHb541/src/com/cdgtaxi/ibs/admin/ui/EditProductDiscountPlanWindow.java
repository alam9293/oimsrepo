...
public class EditProductDiscountPlanWindow extends CommonWindow {
	private static final Logger logger = Logger.getLogger(EditProductDiscountPlanWindow.class);
	...
	public void refresh() throws InterruptedException {
		...
		for (MstbProdDiscDetail detail : sortedDetails) {
			...
			if(DateUtil.getCurrentDate().compareTo(detail.getEffectiveDt()) <=0){
				...
				ZScript showInfo = ZScript.parseContent("editProductDiscountPlanWindow.delete(self.getParent().getParent())");
	...
	public EditProductDiscountPlanWindow() {
	...
	public void onCreate() {
		...
		planNameTB.setValue(plan.getProductDiscountPlanName());
		...
		if(!this.checkUriAccess(Uri.CREATE_PRODUCT_DISCOUNT_PLAN_DETAIL)){
			createBtn.setDisabled(true);
		}	
	}
	
	public void delete(Listitem item) throws InterruptedException {
		if(!this.checkUriAccess(Uri.DELETE_PRODUCT_DISCOUNT_PLAN_DETAIL)){
		...
		try {
			MstbProdDiscDetail detail = (MstbProdDiscDetail)item.getValue();
			this.businessHelper.getAdminBusiness().delete(detail);
			plan = businessHelper.getAdminBusiness().getProductDiscountPlan(plan.getProductDiscountPlanNo());
			MasterSetup.getProductDiscountManager().refresh();
			item.detach();
			Messagebox.show("The Product Discount Plan Detail has been successfully deleted", "Delete Product Discount Plan Detail",
	...
	public void updatePlan() throws InterruptedException {
		plan.setProductDiscountPlanName(planNameTB.getValue());

		try {
			businessHelper.getAdminBusiness().updateProductDiscountPlan(plan, getUserLoginIdAndDomain());
			MasterSetup.getProductDiscountManager().refresh();
			Messagebox.show(
	...
	public void addDetail() throws InterruptedException {
		try {
			...
			forward(Uri.CREATE_PRODUCT_DISCOUNT_PLAN_DETAIL, map);
		}
		catch(Exception e){
			Messagebox.show(com.cdgtaxi.ibs.common.NonConfigurableConstants.COMMON_ERROR_MESSAGE,
					"Error", Messagebox.OK, Messagebox.ERROR);
