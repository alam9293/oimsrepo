...
public class EditSubscriptionFeePlanWindow extends CommonWindow {
	...
	public void refresh() throws InterruptedException {
		...
		for (MstbSubscFeeDetail detail : sortedDetails) {
			...
			if(DateUtil.getCurrentDate().compareTo(detail.getEffectiveDt()) <=0){
				...
				ZScript showInfo = ZScript.parseContent("editSubscriptionFeePlanWindow.delete(self.getParent().getParent())");
	...
	public void delete(Listitem item) throws InterruptedException {
		if(!this.checkUriAccess(Uri.DELETE_SUBSCRIPTION_FEE_PLAN_DETAIL)){
		...
		try {
			MstbSubscFeeDetail detail = (MstbSubscFeeDetail)item.getValue();
			this.businessHelper.getAdminBusiness().delete(detail);
			plan = businessHelper.getAdminBusiness().getSubscriptionFeePlan(plan.getSubscriptionFeeNo());
			MasterSetup.getSubscriptionManager().refresh();
			item.detach();
			Messagebox.show("The Subscription Fee Plan Detail has been successfully deleted", "Delete Subscription Fee Plan Detail",
	...
	public EditSubscriptionFeePlanWindow() {
	...
	public void onCreate() {
		...
		planNameTB.setValue(plan.getSubscriptionFeeName());
		...
		if(!this.checkUriAccess(Uri.CREATE_SUBSCRIPTION_FEE_PLAN_DETAIL)){
			createBtn.setDisabled(true);
		}
	}

	public void updatePlan() throws InterruptedException {
		plan.setSubscriptionFeeName(planNameTB.getValue());

		try {
			businessHelper.getAdminBusiness().updateSubscriptionFeePlan(plan, getUserLoginIdAndDomain());
			MasterSetup.getSubscriptionManager().refresh();
			Messagebox.show(
	...
	public void addDetail() throws InterruptedException {
		try {
			...
			forward(Uri.CREATE_SUBSCRIPTION_FEE_PLAN_DETAIL, map);
		}
		catch(Exception e){
			Messagebox.show(com.cdgtaxi.ibs.common.NonConfigurableConstants.COMMON_ERROR_MESSAGE,
					"Error", Messagebox.OK, Messagebox.ERROR);
