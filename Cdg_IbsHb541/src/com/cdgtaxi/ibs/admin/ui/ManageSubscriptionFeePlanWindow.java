...
public class ManageSubscriptionFeePlanWindow extends CommonWindow {
	private static final Logger logger = Logger.getLogger(ManageSubscriptionFeePlanWindow.class);
	...
	public void onCreate(CreateEvent ce) throws Exception {
		...
		itemTemplate.detach();
		refresh();
		if(!this.checkUriAccess(Uri.CREATE_SUBSCRIPTION_FEE_PLAN)){
	...
	public void editPlan() throws InterruptedException {
		try {
			...
			MstbSubscFeeMaster plan = (MstbSubscFeeMaster)selectedItem.getValue();

			HashMap<String, Object> map = new HashMap<String, Object>();
			map.put("planNo", plan.getSubscriptionFeeNo());
			
			if(this.checkUriAccess(Uri.EDIT_SUBSCRIPTION_FEE_PLAN))
				forward(Uri.EDIT_SUBSCRIPTION_FEE_PLAN, map);
			else if(this.checkUriAccess(Uri.VIEW_SUBSCRIPTION_FEE_PLAN))
				forward(Uri.VIEW_SUBSCRIPTION_FEE_PLAN, map);
			else{
	...
	public void createPlan() throws InterruptedException {
		try {
			forward(Uri.CREATE_SUBSCRIPTION_FEE_PLAN, null);
		} 
		catch(Exception e){
			Messagebox.show(com.cdgtaxi.ibs.common.NonConfigurableConstants.COMMON_ERROR_MESSAGE,
					"Error", Messagebox.OK, Messagebox.ERROR);
	...
	public void deletePlan(Listitem item) throws InterruptedException {
		if(!this.checkUriAccess(Uri.DELETE_SUBSCRIPTION_FEE_PLAN)){
			Messagebox.show(com.cdgtaxi.ibs.common.NonConfigurableConstants.ACCESS_DENIED,
					"Access Denied", Messagebox.OK, Messagebox.EXCLAMATION);
			return;
		}
		...
		try {
			businessHelper.getAdminBusiness().deleteSubscriptionFeePlan((MstbSubscFeeMaster) item.getValue());
			MasterSetup.getSubscriptionManager().refresh();
			item.detach();
			Messagebox.show("Plan has been successfully deleted", "Delete Subscription Fee Plan",
	...
	public void refresh() throws InterruptedException {
		List<MstbSubscFeeMaster> plans = businessHelper.getAdminBusiness().getSubscriptionFeePlans();
		...
		if (plans.size() == 0) {
			...
			for (MstbSubscFeeMaster plan : plans) {
				Listitem item = (Listitem) itemTemplate.clone();
				item.setValue(plan);
				List<Listcell> cells = item.getChildren();
				cells.get(0).setLabel(plan.getSubscriptionFeeName());
				if(plan.getMstbSubscFeeDetails().size()<=0){
					Image deleteImage = new Image("/images/delete.png");
					deleteImage.setStyle("cursor:pointer");
					ZScript showInfo = ZScript.parseContent("manageSubscriptionFeePlanWindow.deletePlan(self.getParent().getParent())");
