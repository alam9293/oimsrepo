...
public class ManageVolumeDiscountPlanWindow extends CommonWindow {
	private static final Logger logger = Logger.getLogger(ManageVolumeDiscountPlanWindow.class);
	...
	public void onCreate(CreateEvent ce) throws Exception {
		...
		if(!this.checkUriAccess(Uri.CREATE_VOLUME_DISCOUNT_PLAN)){
	...
	public void editPlan() throws InterruptedException {
		try {
			...
			MstbVolDiscMaster plan = (MstbVolDiscMaster)selectedItem.getValue();

			HashMap<String, Object> map = new HashMap<String, Object>();
			map.put("planNo", plan.getVolumeDiscountPlanNo());
			
			if(this.checkUriAccess(Uri.EDIT_VOLUME_DISCOUNT_PLAN))
				forward(Uri.EDIT_VOLUME_DISCOUNT_PLAN, map);
			else if(this.checkUriAccess(Uri.VIEW_VOLUME_DISCOUNT_PLAN))
				forward(Uri.VIEW_VOLUME_DISCOUNT_PLAN, map);
			else{
	...
	public void createPlan() throws InterruptedException {
		try {
			forward(Uri.CREATE_VOLUME_DISCOUNT_PLAN, null);
		} 
		catch(Exception e){
			Messagebox.show(com.cdgtaxi.ibs.common.NonConfigurableConstants.COMMON_ERROR_MESSAGE,
					"Error", Messagebox.OK, Messagebox.ERROR);
	...
	public void deletePlan(Listitem item) throws InterruptedException {
		if(!this.checkUriAccess(Uri.DELETE_VOLUME_DISCOUNT_PLAN)){
			Messagebox.show(com.cdgtaxi.ibs.common.NonConfigurableConstants.ACCESS_DENIED,
					"Access Denied", Messagebox.OK, Messagebox.EXCLAMATION);
			return;
		}
		...
		try {
			businessHelper.getAdminBusiness().deleteVolumeDiscountPlan((MstbVolDiscMaster) item.getValue());
			MasterSetup.getVolumeDiscountManager().refresh();
			item.detach();
			Messagebox.show("Plan has been successfully deleted", "Delete Volume Discount Plan",
	...
	public void refresh() throws InterruptedException {
		List<MstbVolDiscMaster> plans = businessHelper.getAdminBusiness().getVolumeDiscountPlans();
		...
		if (plans.size() == 0) {
			...
			for (MstbVolDiscMaster plan : plans) {
				Listitem item = (Listitem) itemTemplate.clone();
				item.setValue(plan);
				List<Listcell> cells = item.getChildren();
				cells.get(0).setLabel(plan.getVolumeDiscountPlanName());
				
				if(plan.getMstbVolDiscDetails().size()<=0){
					Image deleteImage = new Image("/images/delete.png");
					deleteImage.setStyle("cursor:pointer");
					ZScript showInfo = ZScript.parseContent("manageVolumeDiscountPlanWindow.deletePlan(self.getParent().getParent())");
