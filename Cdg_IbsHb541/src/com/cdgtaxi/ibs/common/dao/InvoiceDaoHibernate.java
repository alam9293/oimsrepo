...
public class InvoiceDaoHibernate extends GenericDaoHibernate implements InvoiceDao {
	...
	public List<BmtbInvoiceHeader> getOutstandingInvoiceByAccount(AmtbAccount account) {
		...
		DetachedCriteria invoiceSummaryCriteria = invoiceHeaderCriteria.createCriteria(
				"bmtbInvoiceSummaries", org.hibernate.sql.JoinType.LEFT_OUTER_JOIN);
		...
		invoiceDetailCriteria.createCriteria("pmtbProductType", org.hibernate.sql.JoinType.LEFT_OUTER_JOIN);
	...
	private DetachedCriteria buildSearchInvoiceCriteria(SearchInvoiceForm form) {
		...
		DetachedCriteria receiptDetailsCriteria = invoiceHeaderCriteria.createCriteria(
				"bmtbPaymentReceiptDetails", org.hibernate.sql.JoinType.LEFT_OUTER_JOIN);
		@SuppressWarnings("unused")
		...
		accountCriteria.createAlias("amtbAccount", "p_acct", org.hibernate.sql.JoinType.LEFT_OUTER_JOIN);
		accountCriteria.createAlias("amtbAccount.amtbAccount", "gp_acct", org.hibernate.sql.JoinType.LEFT_OUTER_JOIN);
		accountCriteria.createAlias("amtbAcctStatuses", "acctSts", org.hibernate.sql.JoinType.LEFT_OUTER_JOIN);
		accountCriteria.createAlias("p_acct.amtbAcctStatuses", "p_acctSts", org.hibernate.sql.JoinType.LEFT_OUTER_JOIN);
		accountCriteria.createAlias("gp_acct.amtbAcctStatuses", "gp_acctSts", org.hibernate.sql.JoinType.LEFT_OUTER_JOIN);
	...
	public BigDecimal getLess30DaysTotalOutstandingAmount(AmtbAccount debtToAccount, Date invoiceDate) {
		...
		invoiceHeaderCriteria.add(Restrictions.or(
				Restrictions.sqlRestriction("(? - due_date) <= 30", invoiceDate, StandardBasicTypes.DATE),
	...
	public List<BmtbInvoiceHeader> getInvoicesForPrinting(Integer billGenRequestNo, Integer custNoFrom,
		...
		DetachedCriteria fileCriteria = criteria
				.createCriteria("bmtbInvoiceFile", org.hibernate.sql.JoinType.LEFT_OUTER_JOIN);
		...
		DetachedCriteria acctCriteria = criteria.createCriteria("amtbAccountByAccountNo", "acct",
				org.hibernate.sql.JoinType.LEFT_OUTER_JOIN);
		DetachedCriteria subscribeTo = acctCriteria.createCriteria("amtbSubscTos", org.hibernate.sql.JoinType.LEFT_OUTER_JOIN);
		DetachedCriteria prodType = subscribeTo.createCriteria("comp_id.pmtbProductType", org.hibernate.sql.JoinType.LEFT_OUTER_JOIN);
		DetachedCriteria pAcctCriteria = acctCriteria.createCriteria("amtbAccount", "pacct",
				org.hibernate.sql.JoinType.LEFT_OUTER_JOIN);
		DetachedCriteria psubscribeTo = pAcctCriteria.createCriteria("amtbSubscTos", org.hibernate.sql.JoinType.LEFT_OUTER_JOIN);
		DetachedCriteria pprodType = psubscribeTo.createCriteria("comp_id.pmtbProductType", org.hibernate.sql.JoinType.LEFT_OUTER_JOIN);
		DetachedCriteria gpAcctCriteria = pAcctCriteria.createCriteria("amtbAccount", "gpacct",
				org.hibernate.sql.JoinType.LEFT_OUTER_JOIN);
		DetachedCriteria gpsubscribeTo = gpAcctCriteria.createCriteria("amtbSubscTos", org.hibernate.sql.JoinType.LEFT_OUTER_JOIN);
		DetachedCriteria gprodType = gpsubscribeTo.createCriteria("comp_id.pmtbProductType", org.hibernate.sql.JoinType.LEFT_OUTER_JOIN);
