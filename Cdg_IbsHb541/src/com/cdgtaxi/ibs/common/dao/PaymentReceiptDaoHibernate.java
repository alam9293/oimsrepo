...
public class PaymentReceiptDaoHibernate extends GenericDaoHibernate implements PaymentReceiptDao{
	...
	public BmtbPaymentReceipt getPaymentReceipt(Long receiptNo){
		...
		DetachedCriteria midLevelAccount = lowestLevelAccount.createCriteria("amtbAccount", org.hibernate.sql.JoinType.LEFT_OUTER_JOIN);
		DetachedCriteria topLevelAccount = midLevelAccount.createCriteria("amtbAccount", org.hibernate.sql.JoinType.LEFT_OUTER_JOIN);
		criteria.createCriteria("mstbMasterTableByPaymentMode", DetachedCriteria.INNER_JOIN);
		criteria.createCriteria("mstbMasterTableByCancellationReason", org.hibernate.sql.JoinType.LEFT_OUTER_JOIN);
		criteria.createCriteria("mstbBankMaster", org.hibernate.sql.JoinType.LEFT_OUTER_JOIN);
		criteria.createCriteria("mstbBranchMaster", org.hibernate.sql.JoinType.LEFT_OUTER_JOIN);
		criteria.createCriteria("fmtbBankCode", org.hibernate.sql.JoinType.LEFT_OUTER_JOIN);
		criteria.createCriteria("bmtbPaymentReceipt", org.hibernate.sql.JoinType.LEFT_OUTER_JOIN);
		criteria.createCriteria("bmtbInvoiceDepositTxns", org.hibernate.sql.JoinType.LEFT_OUTER_JOIN);
		DetachedCriteria paymentDetailCriteria = criteria.createCriteria("bmtbPaymentReceiptDetails", org.hibernate.sql.JoinType.LEFT_OUTER_JOIN);
		DetachedCriteria invoiceHeaderCriteria = paymentDetailCriteria.createCriteria("bmtbInvoiceHeader", org.hibernate.sql.JoinType.LEFT_OUTER_JOIN);
		invoiceHeaderCriteria.createCriteria("amtbAccountByDebtTo", org.hibernate.sql.JoinType.LEFT_OUTER_JOIN);
		DetachedCriteria invoiceDetailCriteria = paymentDetailCriteria.createCriteria("bmtbInvoiceDetail", org.hibernate.sql.JoinType.LEFT_OUTER_JOIN);
		invoiceDetailCriteria.createCriteria("pmtbProduct", org.hibernate.sql.JoinType.LEFT_OUTER_JOIN);
		DetachedCriteria invoiceDetailLowestLevelAccount = invoiceDetailCriteria.createCriteria("amtbAccount", org.hibernate.sql.JoinType.LEFT_OUTER_JOIN);
		DetachedCriteria invoiceDetailMidLevelAccount = invoiceDetailLowestLevelAccount.createCriteria("amtbAccount", org.hibernate.sql.JoinType.LEFT_OUTER_JOIN);
		DetachedCriteria invoiceDetailTopLevelAccount = invoiceDetailMidLevelAccount.createCriteria("amtbAccount", org.hibernate.sql.JoinType.LEFT_OUTER_JOIN);
	...
	public List<BmtbPaymentReceipt> getPaymentReceipt(SearchPaymentReceiptForm form){
		...
		DetachedCriteria parentAccountCriteria = accountCriteria.createCriteria("amtbAccount", "parentAccount", org.hibernate.sql.JoinType.LEFT_OUTER_JOIN);
		DetachedCriteria grandParentAccountCriteria = parentAccountCriteria.createCriteria("amtbAccount", "grandParentAccount", org.hibernate.sql.JoinType.LEFT_OUTER_JOIN);
	...
	public void updateReceiptToParent(Integer accountNo, String userId){
		DetachedCriteria paymentReceiptCriteria = DetachedCriteria.forClass(BmtbPaymentReceipt.class);
		paymentReceiptCriteria.createCriteria("amtbAccount", org.hibernate.sql.JoinType.LEFT_OUTER_JOIN)
	...
	public List<BmtbPaymentReceipt> getPaymentReceiptsByInvoice(Long invoiceNo) {
		...
		DetachedCriteria paymentModeCriteria = receiptCriteria.createCriteria("mstbMasterTableByPaymentMode", org.hibernate.sql.JoinType.LEFT_OUTER_JOIN);
		DetachedCriteria cancellationReasonCriteria = receiptCriteria.createCriteria("mstbMasterTableByCancellationReason", org.hibernate.sql.JoinType.LEFT_OUTER_JOIN);
