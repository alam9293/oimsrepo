...
public class PrepaidDaoHibernate extends GenericDaoHibernate implements PrepaidDao{
	...
	public List<Object[]> searchPrepaidInvoiceRequest(SearchPrepaidRequestForm form){
		...
		try{
			...
			if(form.getCardNo().length() != 0) query.setParameter("cardNo", form.getCardNo().toString(), StandardBasicTypes.STRING);
			else query.setParameter("cardNo", null, StandardBasicTypes.STRING);
			
			if(form.getInvoiceNoFrom()!=null) query.setParameter("invoiceNo", form.getInvoiceNoFrom().toString(), StandardBasicTypes.STRING);
			else query.setParameter("invoiceNo", null, StandardBasicTypes.STRING);
			
			if(form.getRequestDateFrom()!=null) query.setParameter("requestDateFromField", form.getRequestDateFrom(), StandardBasicTypes.DATE);
			else query.setParameter("requestDateFromField", null, StandardBasicTypes.DATE);
			
			if(form.getRequestDateTo()!=null) query.setParameter("requestDateToField", form.getRequestDateTo(), StandardBasicTypes.DATE);
			else query.setParameter("requestDateToField", null, StandardBasicTypes.DATE);
	...
	public List<PmtbPrepaidReq> searchPrepaidRequest(SearchPrepaidRequestForm form){
		...
		dc.createCriteria("acct.fmtbArContCodeMaster", "ar", org.hibernate.sql.JoinType.LEFT_OUTER_JOIN);
	...
	public PmtbTopUpReq getPrepaidCreditTopUpRequest(BigDecimal requestNo) {
		...
		dc.createCriteria("ih.bmtbInvoiceSummaries","ihs", org.hibernate.sql.JoinType.LEFT_OUTER_JOIN);
		dc.createCriteria("ihs.bmtbInvoiceDetails","ihsd", org.hibernate.sql.JoinType.LEFT_OUTER_JOIN);
		dc.createCriteria("amtbAccount", "acct", Criteria.LEFT_JOIN);
		dc.createCriteria("acct.fmtbArContCodeMaster", "ar", org.hibernate.sql.JoinType.LEFT_OUTER_JOIN);
		dc.createCriteria("ar.fmtbEntityMaster", org.hibernate.sql.JoinType.LEFT_OUTER_JOIN);
		dc.createCriteria("pmtbTopUpReqCards", "card", Criteria.LEFT_JOIN);
	...
	public PmtbTransferReq getPrepaidTransferRequest(BigDecimal requestNo) {
		...
		dc.createCriteria("amtbAccount", "acct", org.hibernate.sql.JoinType.LEFT_OUTER_JOIN);
		dc.createCriteria("acct.fmtbArContCodeMaster", "ar", org.hibernate.sql.JoinType.LEFT_OUTER_JOIN);
		dc.createCriteria("ar.fmtbEntityMaster", org.hibernate.sql.JoinType.LEFT_OUTER_JOIN);
	...
	public Object[] getLastBalanceForfeitureAsAt(BigDecimal productNo, Date asAtDate){
		...
		try{
			...
			query.setParameter("productNo", productNo, StandardBasicTypes.BIG_DECIMAL);
			query.setParameter("asAtDate", asAtDate, StandardBasicTypes.DATE);
			query.setMaxResults(ConfigurableConstants.getMaxQueryResult());
			results = query.list();
		}
		catch(Exception e){
