...
public class ManageRequestWindow extends CommonWindow implements AfterCompose {

	private static final long serialVersionUID = 1L;
	private static final Logger logger = Logger.getLogger(ManageRequestWindow.class);
	...
	public void afterCompose() {
		...
		for (String key : NonConfigurableConstants.GOVT_EINV_REQUEST_STATUS.keySet()) {
			statusLB.appendChild(new Listitem(NonConfigurableConstants.GOVT_EINV_REQUEST_STATUS.get(key), key));
		}
	}
	...
	public void searchRequest() throws InterruptedException {
		...
		try 
		{
			...
			List<IttbGovtEinvoiceReq> requests = this.businessHelper.getAdminBusiness()
					.searchGovtEInvRequest(form);
			if (requests.size() > 0) {
				...
				for (IttbGovtEinvoiceReq request : requests) {
					...
					item.appendChild(newListcell(NonConfigurableConstants.GOVT_EINV_REQUEST_STATUS
	...
	public void viewRequest() throws InterruptedException {
		...
		try {
			...
			IttbGovtEinvoiceReq request = (IttbGovtEinvoiceReq) selectedItem.getValue();

			HashMap map = new HashMap();
			map.put("requestNo", request.getReqNo());
			this.forward(Uri.VIEW_GOVT_EINV_REQUEST, map);
		} catch (Exception e) {
			Messagebox.show(com.cdgtaxi.ibs.common.NonConfigurableConstants.COMMON_ERROR_MESSAGE, "Error",
					Messagebox.OK, Messagebox.ERROR);
			e.printStackTrace();
