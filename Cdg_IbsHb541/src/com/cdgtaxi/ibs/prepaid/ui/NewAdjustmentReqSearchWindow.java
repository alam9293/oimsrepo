...
public class NewAdjustmentReqSearchWindow extends CommonSearchByAccountWindow implements AfterCompose {

	private Textbox cardNumberStartField, cardNumberEndField, nameOnCardField;
	private Listbox resultList, cardStatusField;
	private static final Logger logger = Logger.getLogger(NewAdjustmentReqSearchWindow.class);
	...
	public void onSelectResultList() throws InterruptedException{
		...
		forward(Uri.NEW_ADJUSTMENT_REQ ,map);
	}
	...
	public void searchPrepaidProducts() throws InterruptedException{
		...
		if(productList.size()>0){
			...
			for(PmtbProduct product : productList){
				...
				item.appendChild(newListcell(StringUtil.bigDecimalToStringWithGDFormat(product.getCardValue())));
				item.appendChild(newListcell(StringUtil.bigDecimalToStringWithGDFormat(product.getCashplus())));
