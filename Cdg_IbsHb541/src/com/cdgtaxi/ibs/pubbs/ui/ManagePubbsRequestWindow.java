...
public class ManagePubbsRequestWindow extends CommonWindow implements AfterCompose {

	private static final long serialVersionUID = 1L;
	private static final Logger logger = Logger.getLogger(ManagePubbsRequestWindow.class);
	...
	public void afterCompose() {
		...
		for (String key : NonConfigurableConstants.PUBBS_REQUEST_STATUS.keySet()) {
			statusLB.appendChild(new Listitem(NonConfigurableConstants.PUBBS_REQUEST_STATUS.get(key), key));
		}
	}
	...
	public void searchRequest() throws InterruptedException {
		...
		SearchPubbsRequestForm form = new SearchPubbsRequestForm();
		...
		try 
		{
			...
			List<IttbPubbsReq> requests = this.businessHelper.getAdminBusiness()
					.searchPubbsRequest(form);
			if (requests.size() > 0) {
				...
				for (IttbPubbsReq request : requests) {
					...
					item.appendChild(newListcell(NonConfigurableConstants.PUBBS_REQUEST_STATUS.get(request.getStatus())));
	...
	public void viewRequest() throws InterruptedException {
		...
		try {
			...
			IttbPubbsReq request = (IttbPubbsReq) selectedItem.getValue();

			HashMap map = new HashMap();
			map.put("requestNo", request.getReqNo());
			this.forward(Uri.VIEW_PUBBS_REQUEST, map);
		} catch (Exception e) {
			Messagebox.show(com.cdgtaxi.ibs.common.NonConfigurableConstants.COMMON_ERROR_MESSAGE, "Error",
					Messagebox.OK, Messagebox.ERROR);
			e.printStackTrace();
