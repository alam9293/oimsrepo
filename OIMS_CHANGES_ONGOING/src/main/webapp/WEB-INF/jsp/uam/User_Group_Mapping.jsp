...
<head>
  ...
  <title>OIMS USER TO GROUP MAPPING</title>
...
{
	var Group = document.getElementById("userNameId").value;
	Group = Group.substr(0,Group.indexOf('$'));
	if(Group==null||Group=="")
	{
		document.getElementById("currentGroupId").value= "No Group Assigned";
	}
	else
	{
		document.getElementById("currentGroupId").value= Group;
	}
...
function validateValues()
{
	...
	if(document.getElementById("userGroupId").selectedIndex==0)
	{
		document.getElementById("userGroupspn").innerHTML ="PLEASE SELECT USER Group";
		return false;
	}
	else
	{
		document.getElementById("userGroupspn").innerHTML ="";
	}	
	return true;
}
</script>
    ...
    <div class="content">
       ...
       <div align ="center">
						<table class="table table-bordered">
							<tbody>
								<form:form modelAttribute ="UsrToGroupMappingModel" onSubmit ="return validateValues()">
									<tr>
										<td>User Name</td>
										<td>
										   <div class="input-group mb-3">
								                <select  class="form-control" id="userNameId" name ="userName" onchange="return validateValues()">
								                  <c:forEach var="groupIdMap" items="${userNameGroupIdMap}" varStatus="counter">
								                     <c:if test ="${counter.index==0}">
								                       <option  value="">Select User Name</option>
								                     </c:if>
								                     <c:set var="arr" value="${fn:substringBefore(groupIdMap.key, '$')}" />
								                     <option value= "${groupIdgroupNameMap[arr]}$${groupIdMap.value}"  >${groupIdMap.value}</option>
								                  </c:forEach>
								                </select>
										    </div>
									...
									<tr>
										<td>Current Group</td>
										<td><input type="text" class="form-control" name="currentGroup" id="currentGroupId" disabled="disabled"></td>
									</tr>
									...
									<tr>
										...
										<td>
										   <div class="input-group mb-3">
								                <form:select path ="" class="form-control select-checkbox" id="userGroupId" name ="userGroup" onchange="return validateValues()">
								                  <c:forEach var="map" items="${grouIdGroupNameMap}" varStatus="counter">
								                     <form:option  value="${map.key}">${map.value}</form:option>
								                  </c:forEach>
								                </form:select>
										    </div>
										    <span id="userGroupspn" class="text-danger color:red"></span>
										 </td>
									</tr>
									 <tr>
										<td colspan=2>
										   <input type="submit"  value ="Add" formaction ="./mapUserwithSelectedGroup"/>
										</td>
									</tr>
								</form:form>
							</tbody>
