...
<script>
function showpopUpandCallAction(action,taskId)
{
	if(action=='Approve')
	{
		var r = confirm("Are you sure, You want to Approve?");
		if (r == true) {
			var action = "./Approve?"+"Task_Id="+taskId;
	...
	if(action=='DisApprove')
	{
		...
		if (r == true) {
			var action = "./disApprove?"+"Task_Id="+taskId;
			location.href = action;
		} else {
			return false
		}
    ...
    <div class="content">
       ...
       <div align ="center" id ="Div1">
				<table class="table table-bordered">
					<tbody>
					 <form:form modelAttribute ="datalistmodel2">
					     <c:forEach var="DataMap" items="${displayDataMap}" varStatus="counter">
					      <c:choose>
					        <c:when test ="${ModuleName=='ROLE MAP FORM'||ModuleName=='ADD ROLE FORM'}">
								<tr>
									 <td>
									   ${DataMap.key}
									 </td>
									 <td>
									   <c:choose>
										   <c:when test ="${DataMap.key=='Allowed Forms'}">
											   <c:set var="formnameParts" value="${fn:split(DataMap.value, ',')}" />
												 <c:forEach var="formsname" items= "${formnameParts}" varStatus="counter2">
												  <ul>
												   <li>
												      <label>${formsname}</label>
												   </li>
												  </ul>
											    </c:forEach>
											    </c:when>
										    <c:otherwise>${DataMap.value}</c:otherwise>
									    </c:choose>
									 </td>
								</tr>	    
					        </c:when>
							<c:otherwise>
							     ...
							     <tr>
									 ...
									 <td colspan =3>
									   <a href ="javascript:showpopUpandCallAction('Approve',${taskId});"  class="common-default-btn mt-3">Approve</a>
									   <a href ="javascript:showpopUpandCallAction('DisApprove',${taskId});" class="common-default-btn mt-3">DisApprove</a>
									   <a href = "./PendingworksApproval" class="common-default-btn mt-3">Back</a>
									  </td>
								 </tr>
				   </tbody>
				</table>
